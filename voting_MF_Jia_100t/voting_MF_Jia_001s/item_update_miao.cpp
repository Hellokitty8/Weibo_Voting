...
void item_update_miao() {//item latent feature update for vote

	/////moved out here, save computation
	//start = time(NULL);
	cout << "item update begin\n";
	...
	for (int i=0; i<dim; i++) {
		for (int j=0; j< dim; j++) {
			...
			qq1[i][j] *= w_m * TS_DEFAULT * TS_DEFAULT;
		}
	}
	...
	for (int m = 0; m <ITEM_NUM; m++) {
		...
		//cout<<"y2.1\n";
		int count = 0;
		for (It = multi_items["voting"][item_id].user_list.begin(); It != multi_items["voting"][item_id].user_list.end(); It++) {
			...
			for(int k = 0; k < dim; k++) {
				...
				first_part[k] += (rate_val - r_m) * users[user_id].user_latent_feature[k] * w * users[user_id].ts_influence[item_id];
			}
			count++;
		}
		...
		for(int i=0; i<dim; i++) {
			...
			for(int j=0; j< dim; j++) {
				...
				for(It = multi_items["voting"][item_id].user_list.begin(); It != multi_items["voting"][item_id].user_list.end(); It++){
					...
					qq2[i][j] += users[*It].user_latent_feature[i] * users[*It].user_latent_feature[j] * users[*It].ts_influence[item_id] * users[*It].ts_influence[item_id] *w;
					qq3[i][j] += users[*It].user_latent_feature[i] * users[*It].user_latent_feature[j];
				}
				...
				qq2[i][j] = qq2[i][j] - w_m * TS_DEFAULT * TS_DEFAULT * qq3[i][j];
			}
		}
		...
		int col = 0;
		for(DIt = multi_items["voting"][item_id].item_latent_feature.begin(); DIt != multi_items["voting"][item_id].item_latent_feature.end(); DIt++, col++ ){
			*DIt = 0;
			for(int m = 0; m< dim; m++) {
